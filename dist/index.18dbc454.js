...
blasts.forEach((blast)=>{
    if (blast.closest(".section-home")) return blastsHome.push(blast);
    if (blast.closest(".section-contact")) return blastsContact.push(blast);
...
const checkBeforeSend = function() {
    ...
    inputField.forEach((el)=>{
        inputValue = el;
        checkInputField(inputValue);
        if (checkInputField() === false) dataErr.push(false);
    });
...
L.marker([
    44.8571126,
    17.5214822
]).addTo(map); //L.marker([44.8571126, 17.5214822]).addTo(map).bindPopup("dsajdj").openPopup;
 //L.marker([44.8571126, 17.5214822], { icon: myIcon }).addTo(map);

},{}]},["jQVXF","1SICI"], "1SICI", "parcelRequireba90")
